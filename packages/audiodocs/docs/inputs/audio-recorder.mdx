...
To access microphone you need to make sure your app has required permission configuration - check [getting started permission section](../fundamentals/getting-started.mdx#special-permissions) for more information.
...
<Tabs queryString="platform">
  <TabItem value="expo" label="Expo" default>
    ...
    For more configuration options, check out the [Expo plugin section](../other/audio-api-plugin.mdx).
...
### Keeping the recording alive when the app is closed <Android /> {#keeping-the-recording-alive-when-the-app-is-closed}
...
<Tabs queryString="platform">
  <TabItem value="expo" label="Expo" default>
    Set the `androidFSStopWithTask` option of the [expo plugin](../other/audio-api-plugin.mdx#androidfsstopwithtask) to `false`:
...
- The foreground service only exists while a library notification is shown. Call [`RecordingNotificationManager.show()`](../system/recording-notification-manager.mdx#show) while the app is still in the foreground — before the user leaves the app — otherwise there is no service to keep alive.
...
:::caution
Even with `stopWithTask="false"`, the system can still kill the process (memory pressure, OEM battery managers). Recording cannot self-restart from the background — the user has to reopen the app. To limit data loss in that case, tune the file-output options [`androidFlushIntervalMs`](#audiorecorderfileoptions) and [`rotateIntervalBytes`](#audiorecorderfileoptions).
:::

A recording that survives task removal can only be controlled from the notification, so give the user a way out: enable the notification's [stop action](../system/recording-notification-manager.mdx#native-action-handling) (`showStopAction: true`) or let them [swipe the notification away](../system/recording-notification-manager.mdx#swiping-the-notification-away) (`dismissible: true`). Both stop the recording natively even when no JS is reachable.
    ...
    <tr>
      ...
      <td>
        <div class="text-content">
          Creates a new `AudioRecorder` instance, optionally configuring the platform capture chain - see [AudioRecorderOptions](#audiorecorderoptions). It is preferred to create only a single instance of `AudioRecorder` for the best performance, memory, and battery consumption. While an idle recorder has minimal impact, switching between separate recorder instances might have a noticeable impact on the device.
        </div>
        ```tsx
...
<!-- -->
    ...
    <tr>
      ...
      <td>
        <div class="text-content">
          Starts the stream from the system audio input device. Returns a `Promise<Result<{}>>` that resolves when recording has started.
        </div>
        ```tsx
        const result = await audioRecorder.start();
...
<!-- -->
    ...
    <tr>
      ...
      <td>
        <div class="text-content">
          Stops the input stream and cleans up each input access method. Returns a `Promise<Result<FileInfo>>` that resolves when recording has stopped.
...
<!-- -->
    ...
    <tr>
      ...
      <td>
        <div class="text-content">
          Returns `true` while a recording session is ongoing (recording or paused). Native source of truth that needs no reference to the recorder instance, so use it from a remounted screen — e.g. after reopening an app whose recording [outlived the app UI](#keeping-the-recording-alive-when-the-app-is-closed) — to seed the UI state. It reflects the recorder that acquired the active session on a successful [start](#start); a second `start()` does not displace it.
        </div>
        ```tsx
          if (AudioRecorder.isRecordingOngoing()) {
            // re-attach the UI to the still-running recording
          }
        ```
      </td>
    </tr>
...
<!-- -->
    ...
    <tr>
      <td><div class="text-content">
        ##### consumeLastRecordingResult
        <br />
        (static)
      </div></td>
      <td>
        <div class="text-content">
          Returns the [FileInfo](#fileinfo) stashed by the last successful stop that produced output files, or `null` if there is none. That includes JS [stop](#stop), the Android [recording notification](../system/recording-notification-manager.mdx#native-action-handling), and [deactivating the iOS audio session](../system/audio-manager.mdx#setaudiosessionactivity) mid-recording. Consume-once: the stash is cleared on read, so a second call returns `null`.

          If the recording was already finalized natively, calling [stop](#stop) on an `AudioRecorder` instance does not return those files — it reports that the recorder is not recording. Read the stashed [FileInfo](#fileinfo) from this method instead.
        </div>
        ```tsx
          const info = AudioRecorder.consumeLastRecordingResult();
          if (info) {
...
<!-- -->
    <tr>
      ...
      <td>
        <div class="text-content">
          `AudioRecorderFileOptions | null` - the active file-output configuration after [enableFileOutput()](#enablefileoutput), or `null` when file output is disabled. See [AudioRecorderFileOptions](#audiorecorderfileoptions).
        </div>
        ```tsx
          const fileOptions = audioRecorder.options;
        ```
      </td>
    </tr>
    <!-- -->
    <tr>
      ...
      <td>
        <div class="text-content">
          `number` - the estimated **full** input latency in seconds (from analog input to app delivery). Mirrors [AudioContext.outputLatency](../core/audio-context.mdx#properties) semantics. Calculated using underlying platform properties (`AVAudioSession` on iOS; Oboe timestamps/burst sizes on Android). Value may change when the audio route changes. Returns `0` when idle.
        </div>
        ```tsx
          const latency = audioRecorder.inputLatency;
        ```
      </td>
    </tr>
  </tbody>
...
<!-- -->
    <tr>
      ...
      <td>
        <div class="text-content">
          Configures and enables file output with the given options and stream properties. By default, the recorder writes to the cache directory using a high-quality `M4A` file. Returns `Result<{}>`.

          Must be called while the recorder is idle: the file is created by the next [start](#start). During an active (recording or paused) session the call returns an error result and the session keeps the output it started with.
    ...
    <tr>
      ...
      <td>
        <div class="text-content">
          Returns the current recording duration (`number`) when file output is enabled.
        </div>
        ```tsx
...
<!-- -->
    <tr>
      ...
      <td>
        <div class="text-content">
          Registers a callback that receives raw audio buffers during an active recording session. Returns `Result<void>`.
...
<!-- -->
    <tr>
      ...
      <td>
        <div class="text-content">
          Routes captured audio into an audio graph by creating a recorder adapter with [BaseAudioContext.createRecorderAdapter()](../core/base-audio-context.mdx#createrecorderadapter), connecting the recorder to it, and wiring it to `destination`. Takes the [BaseAudioContext](../core/base-audio-context.mdx) used to create the recorder adapter and the destination [AudioNode](../core/audio-node.mdx) in the audio graph. Returns [AudioNode](../core/audio-node.mdx).
        </div>
        ```tsx
          audioRecorder.connect(audioContext, audioContext.destination);
        ```
...
- `fileName` - Names the output file outright. Left unset, the library generates `recording_<timestamp>`. See [File naming](#file-naming) below.
...
`fileName` names the output file outright. Left unset, the library generates a name from the
start time, and keeps it clear of existing files. Rotation appends a segment index either way,
because one recording then spans several files.

| `fileName` | `rotateIntervalBytes` | Resulting file(s) |
| :--- | :--- | :--- |
| — | `0` | `recording_20260909_101500.wav` |
| `session-42` | `0` | `session-42.wav` |
| — | `> 0` | `recording_20260909_101500_001.wav`, `_002`, … |
| `session-42` | `> 0` | `session-42_001.wav`, `session-42_002.wav`, … |

The generated timestamp is taken once per recording, in local time, so every segment of one
rotated recording shares it and only the index differs.

`fileName` has to be a bare name: no extension (it follows from `format`), no path
separators and no `..`.

The segment index is padded to three digits and keeps counting beyond that (`_999`,
`_1000`, …). Names stay unique; only their lexicographic order breaks past 999 segments —
`stop()` still returns `paths` in recording order.

An audio route change mid-session (headphones connecting, say) alters the input format on
iOS, but the recording carries on in the same file: only the conversion from the input to
the file's format is rebuilt. A recording without rotation is always a single file.

A generated name never overwrites: when two recordings start within the same second and
would share a timestamp, the later one gets a counter appended
(`recording_20260909_101500_1.wav`, `_2`, …).

:::caution
`fileName` puts you in charge of telling recordings apart. An existing file of the same name
is **overwritten**, with a warning in the native log, and two recordings started under the
same `fileName` overwrite each other — with rotation, segment by segment, since numbering
restarts at `_001` every time. Make the name unique per recording yourself.
:::
...
All encoding is done with platform system APIs — iOS AVFoundation and Android MediaCodec/MediaMuxer. Because each platform exposes a different set of system encoders, format support is platform-specific.
