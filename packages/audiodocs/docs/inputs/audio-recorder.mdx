...
**Static.** Returns `true` while a recording session is ongoing (recording or paused). Native source of truth that needs no reference to the recorder instance, so use it from a remounted screen — e.g. after reopening an app whose recording [outlived the app UI](#keeping-the-recording-alive-when-the-app-is-closed) — to seed the UI state. It reflects the most recently created `AudioRecorder`; constructing another instance mid-recording displaces the probed one.
...
**Static.** Returns the [`FileInfo`](#fileinfo) of a recording that was stopped natively — on Android through the [recording notification](../system/recording-notification-manager.mdx#native-action-handling), on iOS by [deactivating the audio session](../system/audio-manager.mdx#setaudiosessionactivity) mid-recording — or `null` if there is none. Consume-once: the result is cleared on read, so a second call returns `null`.

Recordings stopped through [`stop`](#stop) resolve their promise with the file info instead and never appear here.
