  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        ```
        <div class="text-content">
          Full-duplex voice apps can request the platforms' communication capture paths:
        </div>
        ```tsx
        const audioRecorder = new AudioRecorder({
        ...
        ```
        <div class="text-content">
          These options request platform processing; echo cancellation, noise suppression, and automatic gain control remain device- and route-dependent.
        </div>
      </td>
    </tr>
...
On Android, encoded file output for `M4A`, `FLAC`, and `CAF` uses FFmpeg. When FFmpeg is disabled in the build, only **WAV** recording to file is supported. iOS uses system AVFoundation for all listed formats. See [Runtime flags](../other/runtime-flags.mdx#where-ffmpeg-is-used).
:::
