...
Use them to branch UI or loading logic — for example, skip remote metadata preload when FFmpeg is disabled, hide streaming controls when [`StreamerNode`](/docs/sources/streamer-node) is unavailable, or offer only WAV recording on Android.
...
| :--- | :--- | :--- |
| Streaming | [`StreamerNode`](/docs/sources/streamer-node) | HLS playback |
