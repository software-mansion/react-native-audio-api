...
`WorkletProcessingNode` is an effect node that processes audio synchronously on the **audio worklet runtime** each render quantum. Unlike [`WorkletNode`](./worklet-node.mdx), which only provides read-only snapshots on the UI runtime, this node receives input audio and writes processed output back into the graph.
...
- [`WorkletSourceNode`](./worklet-source-node.mdx) — generate audio with a worklet
