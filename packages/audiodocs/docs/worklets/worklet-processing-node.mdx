...
The `WorkletProcessingNode` interface represents a node in the audio processing graph that can process audio using a worklet function. Unlike `WorkletNode` which only provides read-only access to audio data, `WorkletProcessingNode` allows you to modify the audio signal by providing both input and output buffers.
...
[`BaseAudioContext.createWorkletProcessingNode(worklet, runtimeKind)`](/docs/core/base-audio-context#createworkletprocessingnode-)
...
This is happening because the microtask queue is not always being flushed properly.
...
This will ensure that the microtask queue is flushed and your UI will be updated properly. But be aware that this might have some performance implications so it is not included by default.
