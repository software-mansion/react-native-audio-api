...
```
> **Note**: You must install `react-native-worklets` version above or equal to `0.5.1`. This is very important. For maximum compatibility, we recommend using exactly version `0.5.1` for now as we are testing on this version.
