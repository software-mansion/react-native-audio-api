...
| `context` | `BaseAudioContext` | — | Optional. Native: implicit [`AudioContext`](../core/audio-context.mdx) when omitted. Web: unused for HTML element playback. |
...
| `volume` | `number` | `1.0` | Linear volume passed to the underlying source. |
...
| `playbackRate` | `number` | `1.0` | Playback speed. Native playback accepts rates up to `4.0`×; negative reverse playback is not supported. No-op for HLS files playback. |
...
For remote `http(s):` sources, duration is read through the same native URL probing path as [`getAudioDuration`](../utils/decoding.mdx#getaudioduration). This requires an FFmpeg-enabled build — check with [`isFfmpegEnabled`](../other/runtime-flags.mdx#isffmpegenabled).
...
- **FFmpeg:** Remote URL streaming (HTTP byte ranges) and HLS require an FFmpeg build. Without FFmpeg, remote progressive sources are downloaded fully and decoded with OS APIs / miniaudio where supported. See [Runtime flags](../other/runtime-flags.mdx#where-ffmpeg-is-used).
