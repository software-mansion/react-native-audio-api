...
Removes a buffer from the queue. Note that `onBufferEnded` event will not be fired for buffers that were removed.
...
Sets (or remove) callback that will be fired when a specific buffer has ended with payload `{ bufferId: <bufferId>, isLastBufferInQueue: <isLastBufferInQueue> }`,
where `bufferId` is the ID of the buffer that has ended and `isLastBufferInQueue` is a boolean indicating whether it was the last buffer in the queue.

You can remove callback by passing `null`.

```ts
audioBufferSourceNode.isLastBufferInQueue = (event) => { //setting callback
