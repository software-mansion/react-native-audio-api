...
Declares that no further buffers will be enqueued. Once the queue empties after this call, the node treats that as end-of-stream: with [`pitchCorrection`](#constructor) enabled it flushes the WSOLA OLA tail (same drain path as [`AudioBufferSourceNode`](/docs/sources/audio-buffer-source-node)), then finishes and fires [`onEnded`](/docs/sources/audio-scheduled-source-node#onended). Without this call, an empty queue is only an underrun — the node stays alive and waits for more buffers.

Call after the last `enqueueBuffer` (before or after `start`):

```tsx
queue.enqueueBuffer(chunk1);
queue.enqueueBuffer(chunk2);
queue.endOfStream();
queue.start();
```
