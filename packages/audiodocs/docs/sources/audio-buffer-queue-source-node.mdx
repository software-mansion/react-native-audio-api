...
Adds another buffer to queue. Returns `bufferId` that can be used to identify the buffer in [`onBufferEnded`](/docs/sources/audio-buffer-queue-source-node#onbufferended) event.
...
Removes a buffer from the queue. Note that [`onBufferEnded`](/docs/sources/audio-buffer-queue-source-node#onbufferended) event will not be fired for buffers that were removed.
...
Removes all buffers from the queue. Note that [`onBufferEnded`](/docs/sources/audio-buffer-queue-source-node#onbufferended) event will not be fired for buffers that were removed.
...
Sets (or remove) callback that will be fired when a specific buffer has ended with payload of type [`OnBufferEndEventType`](/docs/sources/audio-buffer-queue-source-node#onbufferendeventtype)
