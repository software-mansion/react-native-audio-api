...
Allows to add another buffer to queue. Returns `bufferId` that can be used to identify the buffer in `onEnded` event.
...
Allows to remove a buffer from the queue. Note that `onEnded` event will not be fired for buffers that were removed.
...
Allows to remove all buffers from the queue. Note that `onEnded` event will not be fired for buffers that were removed.
