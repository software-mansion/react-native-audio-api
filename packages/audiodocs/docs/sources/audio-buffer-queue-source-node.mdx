...
`AudioBufferQueueSourceNode(context: BaseAudioContext, options?: AudioBufferQueueSourceOptions)`
...
| `offset` <Optional /> | `number` | The offset, in seconds, from the start of the first buffer in the queue. Playback begins at this position within that buffer; buffers that follow always play from the start. <br /> The default value is `0`, which starts playback from the beginning of that buffer. If the offset exceeds the buffer’s [`duration`](/docs/sources/audio-buffer#properties), it’s automatically clamped to the valid range. |
