...
| `frequency` <Optional /> | `number` | `440.0` | Initial value for [`frequency`](/docs/sources/oscillator-node#properties). |
| `detune` <Optional /> | `number` | `0.0` | Initial value for [`detune`](/docs/sources/oscillator-node#properties). |

:::info
You can also create an `OscillatorNode` via [`BaseAudioContext.createOscillator()`](/docs/core/base-audio-context#createoscillator), which uses default values.
:::
...
| :----: | :----: | :----: | :------- |
| `detune` <ReadOnly /> | [`AudioParam`](/docs/core/audio-param) | `0.0` |[`a-rate`](/docs/core/audio-param#a-rate-vs-k-rate) `AudioParam` representing detuning of oscillation in cents. |
| `frequency` <ReadOnly /> | [`AudioParam`](/docs/core/audio-param) | `440.0` | [`a-rate`](/docs/core/audio-param#a-rate-vs-k-rate) `AudioParam` representing frequency of wave in herzs. |
| `type` | [`OscillatorType`](/docs/types/oscillator-type)| `sine` | String value represening type of wave. |

## Methods

Inherits all methods from [`AudioScheduledSourceNode`](/docs/sources/audio-scheduled-source-node#methods).
