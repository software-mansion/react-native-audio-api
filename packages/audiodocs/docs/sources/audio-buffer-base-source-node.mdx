...
The `AudioBufferBaseSourceNode` interface is an [`AudioScheduledSourceNode`](/docs/sources/audio-scheduled-source-node) that defines shared behavior for source nodes backed by an [`AudioBuffer`](/docs/sources/audio-buffer).
...
Typically around `0.06s` when pitch correction is enabled, and `0.0` otherwise.
