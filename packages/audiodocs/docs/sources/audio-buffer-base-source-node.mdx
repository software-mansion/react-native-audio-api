...
Typically around `0.06` s when pitch correction is enabled, and `0.0` otherwise.
...
- When created with `pitchCorrection` it is clamped to range `-4.0` to `4.0` and uses pitch correction algorithm.
