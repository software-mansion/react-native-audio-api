...
It can be created from audio file using [`decodeAudioData`](/docs/utils/decoding#decodeaudiodata) or from raw data using the [constructor](#constructor).
...
| `numberOfChannels` <Optional /> | `number` | `1` | Number of [`channels`](/docs/sources/audio-buffer#properties) in buffer |
