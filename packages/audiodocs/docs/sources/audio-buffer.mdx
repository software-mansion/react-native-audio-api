...
It can be created from audio file using [`decodeAudioData`](/docs/utils/decoding#decodeaudiodata) or from raw data using [`AudioBuffer(options)`](#constructor).
Once you have data in `AudioBuffer`, audio can be played by passing it to [`AudioBufferSourceNode`](audio-buffer-source-node).

## Constructor

`AudioBuffer(options: AudioBufferOptions)`
