...
It can contain one or more channels, each one appearing to be 32-bit floating-point linear [PCM](https://pl.wikipedia.org/wiki/PCM) values with a nominal range of [−1,1] (but not limited to that range).
