...
`RecorderAdapterNode(context: BaseAudioContext)`

:::info
You can also create a `RecorderAdapterNode` via [`BaseAudioContext.createRecorderAdapter()`](/docs/core/base-audio-context#createrecorderadapter), which uses default values.
:::
