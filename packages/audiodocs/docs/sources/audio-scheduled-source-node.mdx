...
The `AudioScheduledSourceNode` interface is an [`AudioNode`](../core/audio-node.mdx) which serves as a parent interface for several types of audio source nodes.
It provides ability to start and stop audio playback.

Child classes:
 - [`AudioBufferSourceNode`](./audio-buffer-source-node.mdx)
 - [`AudioBufferQueueSourceNode`](./audio-buffer-queue-source-node.mdx)
 - [`OscillatorNode`](./oscillator-node.mdx)
