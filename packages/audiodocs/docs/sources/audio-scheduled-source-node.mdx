...
Allows to schedule the node to start audio playback at specified time, if no time is given, it starts immediately.
...
Allows to schedule the node to stop audio playback at specified time, if no time is given, it stops immediately.
