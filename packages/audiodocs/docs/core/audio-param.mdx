...
The `AudioParam` interface represents audio-related parameter (such as `gain` property of [GainNode`](../effects/gain-node.mdx)).
...
(such as [`linearRampToValueAtTime`](./audio-param.mdx#linearramptovalueattime) or [`exponentialRampToValueAtTime`](./audio-param.mdx#exponentialramptovalueattime)),
as they are more efficient for continuous changes. For more specific use cases, you can schedule multiple value changes using [`setValueCurveAtTime`](./audio-param.mdx#setvaluecurveattime).
...
| `NotSupportedError` | `startTime` falls within the interval $[T, T+D)$ where $T$ is the time of previously scheduled [`setValueCurveAtTime`](./audio-param.mdx#setvaluecurveattime) event and $D$ is its duration. |
...
| `NotSupportedError` | `endTime` falls within the interval $[T, T+D)$ where $T$ is the time of previously scheduled [`setValueCurveAtTime`](./audio-param.mdx#setvaluecurveattime) event and $D$ is its duration. |
...
This method is useful for decay or release portions of [ADSR envelopes](../effects/gain-node.mdx#advanced-usage--envelope-adsr).
...
Schedules parameters's value change following a curve defined by given array.
...
All time parameters should be in the same time coordinate system as [`BaseAudioContext.currentTime`](./base-audio-context.mdx).
