...
| Error type | Description |
| :---: | :---- |
| `TypeError` | `input` is not a module id, URI, or `ArrayBuffer`. |
| `AudioApiError` | `input` is a base64 or data-blob string (use [`decodePCMInBase64`](#decodepcminbase64) for raw PCM base64). |
