...
| Parameter | Type | Description |
| :---: | :---: | :---- |
| `options` <Optional /> | [`BiquadFilterNodeOptions`](/docs/effects/biquad-filter-node#constructor) | Dictionary object that specifies the initial values of the biquad filter properties. All members are optional; if not specified, the normal default values are used to construct the node. |
