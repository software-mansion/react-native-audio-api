...
The callback receives no arguments; read the new value from the context's [`state`](./base-audio-context.mdx#properties) property, which is already updated when the callback runs.

```tsx
const audioContext = new AudioContext();

audioContext.onstatechange = () => {
  console.log(`state is now ${audioContext.state}`);
};
