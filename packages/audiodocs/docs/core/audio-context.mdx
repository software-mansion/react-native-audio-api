...
| :----: | :----: | :-------- | :-: |
| `baseLatency` | `number` | Processing latency in seconds incurred by the `AudioContext` passing audio from the `AudioDestinationNode` to the audio subsystem. Does not include hardware output latency or audio graph processing time. Equals the render callback duration (`callbackFrames / sampleRate`); falls back to the platform buffer duration before the first callback fires. Returns `0` when the context is not running. | <ReadOnly /> |
