...
- **Create AudioContext with initSuspended**: When creating a new instance of `AudioContext`, consider setting the `initSuspended` option to `true`.
  This ensures that the audio context starts in a suspended state. This is particularly useful for applications that do not want to interrupt other audio applications upon launch.
