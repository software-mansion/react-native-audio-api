...
Add `react-native-audio-api` expo plugin to your `app.json` or `app.config.js`. Read more about the plugin [here](/docs/other/audio-api-plugin)!
...
If you plan to use any of [`WorkletNode`](/docs/worklets/worklet-node), [`WorkletSourceNode`](/docs/worklets/worklet-source-node), or [`WorkletProcessingNode`](/docs/worklets/worklet-processing-node), it is required to have the `react-native-worklets` library set up with version **0.6.0** or higher. See the [worklets getting started page](https://docs.swmansion.com/react-native-worklets/docs/) for info on how to do it.
...
In [the next section](/docs/guides/making-a-piano-keyboard), we will learn how the audio graph works and build a simple piano keyboard.
