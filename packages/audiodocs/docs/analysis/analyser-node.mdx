...
| :---: | :---: | :----: | :---- |
| `fftSize` <Optional /> | `number` | `2048` | Initial value for [`fftSize`](/docs/analysis/analyser-node#properties). |
| `minDecibels` <Optional /> | `number` | `-100.0` | Initial value for [`minDecibels`](/docs/analysis/analyser-node#properties). |
| `maxDecibels` <Optional /> | `number` | `-30.0` | Initial value for [`maxDecibels`](/docs/analysis/analyser-node#properties). |
| `smoothingTimeConstant` <Optional /> | `number` | `0.8` | Initial value for [`smoothingTimeConstant`](/docs/analysis/analyser-node#properties). |

You can also create an `AnalyserNode` via the [`BaseAudioContext.createAnalyser()`](/docs/core/base-audio-context#createanalyser) factory method, which uses default values.
...
- When getting data from [`getByteFrequencyData()`](/docs/analysis/analyser-node#getbytefrequencydata), any frequency with amplitude lower than `minDecibels` will be returned as `0`.
- Throws `IndexSizeError` if the provided value is greater than or equal to `maxDecibels`.

#### `maxDecibels`
- `0.0` dB ([decibel](https://en.wikipedia.org/wiki/Decibel)) is the loudest possible sound, `-10.0` dB is a 10th of that.
- When getting data from [`getByteFrequencyData()`](/docs/analysis/analyser-node#getbytefrequencydata), any frequency with amplitude higher than `maxDecibels` will be returned as `255`.
