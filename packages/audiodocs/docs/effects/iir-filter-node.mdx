...
It is an [`AudioNode`](../core/audio-node.mdx) used for tone controls, graphic equalizers, and other audio effects.
`IIRFilterNode` lets the parameters of the filter response be specified, so that it can be tuned as needed.

In general, it is recommended to use [`BiquadFilterNode`](./biquad-filter-node.mdx) for implementing higher-order filters,
...
You can also create an `IIRFilterNode` via the [`BaseAudioContext.createIIRFilter()`](../core/base-audio-context.mdx#createiirfilter) factory method.
...
| `InvalidStateError` | All of the feedforward coefficients are `0.0`, or the first feedback coefficient is `0.0`. |
