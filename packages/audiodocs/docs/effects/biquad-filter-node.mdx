...
This interactive example shows effects you can achieve using `BiquadFilterNode`: a `lowpass` filter whose [`frequency`](/docs/effects/biquad-filter-node#properties) follows the player cover, muffling the audio while it is closed and opening the sound as you raise it.
...
| `type` <Optional /> | [`BiquadFilterType`](#biquadfiltertype-enumeration-description) | `lowpass` | Initial value for [`type`](/docs/effects/biquad-filter-node#properties). |

:::info
You can also create a `BiquadFilterNode` via [`BaseAudioContext.createBiquadFilter()`](/docs/core/base-audio-context#createbiquadfilter), which uses default values.
:::
