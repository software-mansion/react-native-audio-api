...
:::caution
Linear convolution is a heavy computational process, so if your audio has some weird artefacts that should not be there, try to decrease duration of impulse response buffer. 
Responses that have duration greater than `sampleRate` are not recommended, however it all depends on your specific use-case.
:::