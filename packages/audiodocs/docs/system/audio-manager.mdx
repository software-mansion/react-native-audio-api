...
| :----: | :----: | :-------- |
| `audioEventEmitter` <ReadOnly /> | `AudioEventEmitter` | Custom event emiter that allows to create listener to events, that are invoked from a system. |
...
| `enabled` | `boolean` | Indicates the start or the end of event emition |
