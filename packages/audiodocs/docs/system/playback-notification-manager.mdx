...
function AudioPlayer() {
  useEffect(() => {
    ...
    PlaybackNotificationManager.register();
  ...
  const updateProgress = (elapsedTime: number) => {
    PlaybackNotificationManager.update({ elapsedTime });
  };
}
```
...
<details>
  <summary>Type definitions</summary>
  ```typescript
  type PlaybackNotificationEventName =
    | 'playbackNotificationPlay'
    | 'playbackNotificationPause'
