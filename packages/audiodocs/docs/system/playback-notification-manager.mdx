...
await PlaybackNotificationManager.show({
  ...
  duration: 180,
  state: 'paused',
  skipInterval: 30, // seconds to skip on fast-forward/rewind; default: 15
});
...
const skipForwardListener = PlaybackNotificationManager.addEventListener(
  ...
  (event) => {
    // event.value is the skip interval in seconds
    seekBy(event.value);
  }
);
...
- On Android, notification action icons may still display "15" regardless of the configured interval.
