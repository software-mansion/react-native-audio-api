...
The `RecordingNotificationManager` provides system integration with [`AudioRecorder`](../inputs/audio-recorder.mdx) on Android.
...
- **Stop** (`showStopAction: true`) stops the recorder and finalizes the output files (their info becomes available through [`AudioRecorder.consumeLastRecordingResult()`](../inputs/audio-recorder.mdx#consumelastrecordingresult)), emits `recordingNotificationStop`, then hides the notification, which also stops the foreground service. Unlike pause/resume, your `recordingNotificationStop` listener should **not** call `AudioRecorder.stop()` — the recording is already stopped. Collect the files with `AudioRecorder.consumeLastRecordingResult()` instead.

## Swiping the notification away

By default the notification is pinned: it cannot be swiped away while a recording is in progress or paused. Android 13 and older honour this through the ongoing flag; on Android 14 and newer, where the system lets users dismiss ongoing notifications, it is re-posted immediately after a swipe. Once the recorder is idle, a swipe is honoured — call `hide()` when you are done so the foreground service stops as well.

Set `dismissible: true` to let the user swipe the notification away instead. A swipe then behaves exactly like the stop action: the recording is stopped natively and its files finalized, `recordingNotificationStop` is emitted, and the foreground service stops. Android 12 and older never let the user dismiss a foreground-service notification, so the option only takes effect on Android 13 and newer.
...
interface RecordingNotificationInfo {
  ...
  dismissible?: boolean; // a swipe stops the recording instead of re-posting the notification, default: false
