...
| :----: | :----: | :---- |
| `availableInputs` | [`AudioDeviceInfo[]`](/docs/system/audio-manager#audiodeviceinfo) | List of available audio input devices under the current device configuration. |
| `currentInput` | [`AudioDeviceInfo`](/docs/system/audio-manager#audiodeviceinfo) \| `null` | The currently selected audio input device, or `null` if none is yet selected by the system. |
