...
| :----: | :----: | :-------- |
| `fftSize` | `number` | Returns long value representing size of [Fast Fourier Transform](https://en.wikipedia.org/wiki/Fast_Fourier_transform) used to determine frequency domain. In general it is size of returning time-domain data. |
| `minDecibels` | `number` | Returns float value representing the minimum value for the range of results from `getByteFrequencyData()`. |
| `maxDecibels` | `number` | Returns float value representing the maximum value for the range of results from `getByteFrequencyData()`. |
...
| Name | Type | Description |
| :----: | :----: | :-------- |
| `frequencyBinCount` | `number` | Returns long value representing the half of the `fftSize`. In general it is size of returning frequency-domain data. |
