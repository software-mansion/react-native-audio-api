...
In this guide, we will edit files in order to achieve [`GainNode`](../effects/gain-node.mdx) functionality.
For the sake of a simplicity, we will use value as a raw `double` type, not wrapped in [`AudioParam`](../core/audio-param.mdx).
