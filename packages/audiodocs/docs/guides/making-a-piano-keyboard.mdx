...
In this section, we will use some of the core Audio API interfaces to create a simple piano keyboard. We will learn what an [`AudioParam`](../core/audio-param.mdx) is and how to use it to change the pitch of the sound.

## Base application

We will start with a simple app with a couple of buttons so we don't need to worry about the UI later.
...
## Audio context and sample preloading
...
const sourceList: PR<string> = {
  A: 'https://software-mansion.github.io/react-native-audio-api/audio/sounds/A3.mp3',
  C: 'https://software-mansion.github.io/react-native-audio-api/audio/sounds/C4.mp3',
  E: 'https://software-mansion.github.io/react-native-audio-api/audio/sounds/E4.mp3',
};
```
...
Now it is finally time to play the sounds. We will use the [`AudioBufferSourceNode`](../sources/audio-buffer-source-node.mdx) and simply play the buffers.
...
To stop the sound when keys are released, we will need to store somewhere source nodes, in order to be able to call [`stop`](../sources/audio-scheduled-source-node.mdx#stop) on them later. Just like with the audio context, let's use the `useRef` hook for this.
...
We will start from the end this time, and finally, we will use new type of audio node - [`GainNode`](../effects/gain-node.mdx) :tada:.
`GainNode` is a simple node that can change the volume of any node (or nodes) connected to it. The `GainNode` has a single element called [`AudioParam`](../core/audio-param.mdx), which is also named `gain`.
...
- [`setValueAtTime`](../core/audio-param.mdx#setvalueattime)
- [`exponentialRampToValueAtTime`](../core/audio-param.mdx#exponentialramptovalueattime).
...
With all the knowledge we have gathered, let's get back to the code. In our `onKeyPressIn` function, besides creating the source node, we will create a [`GainNode`](../effects/gain-node.mdx) which will stand in the middle between the source and destination nodes, acting as our envelope. <br /> <br />
...
The [`AudioBufferSourceNode`](../sources/audio-buffer-source-node.mdx) also has its own [`AudioParam`](../core/audio-param.mdx), called `playbackRate` as the title suggests. It allows us to change the speed of the playback of the audio buffer.
...
Our five keys span the notes from `A3` up to `E4`, so these are the frequencies we will base our calculations on:

```tsx
const noteToFrequency = {
  A: 220.0, // A3
  B: 246.942, // B3
  C: 261.626, // C4 - middle C
  D: 293.665, // D4
  E: 329.628, // E4
};
```
...
const onKeyPressIn = (which: KeyName) => {
  ...
  if (!buffer) {
    ...
    buffer = bufferMapRef.current[closestKey];
...
- [`AudioParam`](../core/audio-param.mdx) is an interface that provides ways to control various aspects of audio nodes over time
- [`GainNode`](../effects/gain-node.mdx) is a simple node that can change the volume of any node connected to it
- [`AudioBufferSourceNode`](../sources/audio-buffer-source-node.mdx) has a parameter called `playbackRate` that allows to change the speed of the audio buffer's playback, thereby altering the pitch of the sound
...
In [the next section](./noise-generation.mdx), we will learn how we can generate noise using the audio buffer source node.
