# packages/audiodocs/ — Documentation Site

Docusaurus 3 static site. Docs are MDX files in `docs/`. Custom React components live in `src/components/`.

## Commands

```bash
yarn start   # dev server with hot reload
yarn build   # production build
yarn serve   # serve the built site
```

## Directory Structure

```
packages/audiodocs/
├── docs/                  # "Next" version — all documentation pages (MDX), edited here
│   ├── fundamentals/      # Getting started, installation, best practices
│   ├── guides/            # Tutorials (piano keyboard, noise, ADSR envelope)
│   ├── core/              # AudioContext, AudioNode, AudioParam, BaseAudioContext
│   ├── sources/           # Oscillator, BufferSource, ConstantSource, MediaElement
│   ├── effects/           # Gain, BiquadFilter, Convolver, Dynamics, StereoPanner, Delay, IIR...
│   ├── analysis/          # AnalyserNode
│   ├── inputs/            # AudioRecorder
│   ├── destinations/      # AudioDestinationNode
│   ├── system/            # AudioManager, PlaybackNotificationManager
│   ├── worklets/          # AudioWorklet setup and nodes
│   ├── types/             # Enum docs (OscillatorType, BiquadFilterType...)
│   ├── utils/             # Utility functions (decoding, file utils)
│   ├── react/             # React-specific hooks and helpers
│   └── experimental/      # Experimental APIs
├── src/
│   ├── components/        # Custom MDX components
│   ├── examples/          # Standalone interactive demo implementations
│   └── pages/             # Landing page
├── static/                # Images, audio files
├── versioned_docs/        # "Latest" version — snapshot of docs/, re-cut at release time
├── versioned_sidebars/    # Sidebars per released version
├── versions.json          # List of released versions
├── sidebars.js            # Autogenerated — no manual edits needed
└── docusaurus.config.js   # Site config
```

## Versioning

`main` is the only source of truth for documentation. The site ships two versions, configured
under `presets.classic.docs.versions`:

| Version | Source | URL | Notes |
|---|---|---|---|
| `Latest` | `versioned_docs/version-latest/` | `/docs/...` | Default; snapshot of `docs/` taken at the last npm release |
| `Next` | `docs/` | `/docs/next/...` | Tracks `main`; shows an "unreleased" banner |

**Edit `docs/`.** `versioned_docs/version-latest/` is a snapshot of it, re-cut at release time, so
anything landed in `docs/` reaches `Latest` on the next release. Edit the snapshot directly only
when a fix has to reach live `Latest` readers before that release — and make the same edit in
`docs/`, otherwise the re-cut drops it.

### Re-cutting `Latest` from `docs/`

```bash
cd <repo root>
node -e "require('fs').rmSync('packages/audiodocs/versioned_docs/version-latest',{recursive:true,force:true})"
mkdir -p packages/audiodocs/versioned_docs/version-latest
cp -R packages/audiodocs/docs/. packages/audiodocs/versioned_docs/version-latest/
```

Run this at release time, not per PR, from the commit being released.

### Cross-document links must be relative in `docs/`

In `docs/` (the `Next` version), use relative file paths ending in `.mdx`, never absolute
`/docs/...` URLs:

```mdx
[`AudioParam`](../core/audio-param.mdx)              ✅ resolves within the reader's version
[`gain`](./gain-node.mdx#properties)                 ✅ anchors work the same way
[`AudioParam`](/docs/core/audio-param)               ❌ always jumps to Latest, even from Next
```

Absolute `/docs/...` paths are still correct for static assets under `static/docs/`
(e.g. `![](/docs/recorder/info-plist.png)`).

Parts of `versioned_docs/version-latest/` still use absolute `/docs/...` links, inherited from an
older snapshot. That is harmless there — `Latest` is served at `/docs/`, so those links stay inside
their own version. Do not "fix" them in the snapshot; fix `docs/` and let the next re-cut carry it.

`onBrokenLinks` and `onBrokenAnchors` are set to `throw`, so `yarn build` catches a bad relative
path — but it cannot catch an absolute link leaking a Next reader into Latest.

An absolute link is also how a `Next`-only anchor breaks the build: the target section exists in
`docs/` but not yet in the `Latest` snapshot it links into, so `onBrokenAnchors` throws.

### Anchors come from headings only

Docusaurus collects link targets from heading ids. A hand-rolled `<span id="foo"></span>` renders
but is not collected, so `](#foo)` fails the build. To link to something smaller than a section,
link to the heading that contains it.

A badge in a heading also leaks into its slug — `### \`takeLastRecordingResult\` <Android />`
becomes `#takelastrecordingresult-`, with a trailing hyphen. Pin the anchor explicitly instead:

```mdx
### `takeLastRecordingResult` <Android /> {#takelastrecordingresult}
```

## Sidebar / Navigation

Sidebar is **fully autogenerated** from the folder structure — no edits to `sidebars.js` needed.

Order is controlled by:
- **Between folders**: `_category_.json` → `position` field
- **Within a folder**: frontmatter `sidebar_position: N` in each `.mdx` file

To add a new category folder, create `_category_.json`:
```json
{ "label": "My Category", "position": 6 }
```

## MDX File Format

All doc pages are `.mdx` files with YAML frontmatter.

### Minimal frontmatter

```yaml
---
sidebar_position: 3
---
```

### Full page structure for an audio node

```mdx
---
sidebar_position: 3
---

import AudioNodePropsTable from "@site/src/components/AudioNodePropsTable"
import AudioNodeMethodsTable from "@site/src/components/AudioNodeMethodsTable"
import { Optional, ReadOnly } from '@site/src/components/Badges';

# GainNode

Brief one-line description. `GainNode` extends [`AudioNode`](/docs/core/audio-node).

## Constructor

```tsx
constructor(context: BaseAudioContext, options?: GainOptions)
```

### `GainOptions`

Inherits all properties from [`AudioNodeOptions`](/docs/core/audio-node#audionodeoptions).

| Parameter | Type | Default | |
| :---: | :---: | :----: | :---- |
| `gain` <Optional /> | `number` | `1.0` | Initial value for [`gain`](/docs/effects/gain-node#properties) |

Or by using [`BaseAudioContext.createGain()`](/docs/core/base-audio-context#creategain).

## Example

```tsx
import { AudioContext } from 'react-native-audio-api';

const ctx = new AudioContext();
const gain = ctx.createGain();
gain.gain.value = 0.5;
gain.connect(ctx.destination);
```

## Properties

Inherits all properties from [`AudioNode`](/docs/core/audio-node#properties).

<AudioNodePropsTable
  numberOfInputs={1}
  numberOfOutputs={1}
  channelCount={2}
  channelCountMode="max"
  channelInterpretation="speakers"
/>

| Name | Type | Description | |
| :----: | :----: | :-------- | :-: |
| `gain` | [`AudioParam`](/docs/core/audio-param) | [`a-rate`](/docs/core/audio-param#a-rate-vs-k-rate) `AudioParam` representing the gain value to apply. | <ReadOnly /> |

## Methods

Inherits all methods from [`AudioNode`](/docs/core/audio-node#methods).

<AudioNodeMethodsTable />

`GainNode` does not define any additional methods.

## Remarks

#### `gain`
- Nominal range is -∞ to ∞.
- Values greater than `1.0` amplify the signal; values between `0.0` and `1.0` attenuate it.
- A value of `0.0` silences the signal. Negative values invert the signal phase.
```

## Custom MDX Components

### Badges — inline in tables or text

```tsx
import { Optional, ReadOnly, Overridden, IOS, Android, Experimental, MobileOnly }
  from '@site/src/components/Badges';

| `gain` <ReadOnly /> | `AudioParam` | `1.0` | ... |
| `type` <Optional /> | `string` | `'sine'` | ... |
```

### AudioNodePropsTable — standard node property summary

```tsx
import AudioNodePropsTable from "@site/src/components/AudioNodePropsTable"

<AudioNodePropsTable
  numberOfInputs={1}
  numberOfOutputs={1}
  channelCount={2}
  channelCountMode={"max"}
  channelInterpretation={"speakers"}
/>
```

Use the values from the Web Audio API spec for the node you are documenting.

### AudioNodeMethodsTable — inherited `connect` / `disconnect`

```tsx
import AudioNodeMethodsTable from "@site/src/components/AudioNodeMethodsTable"

## Methods

Inherits all methods from [`AudioNode`](/docs/core/audio-node#methods).

<AudioNodeMethodsTable />

`MyNode` does not define any additional methods.
```

Use in every node page that inherits directly from [`AudioNode`](/docs/core/audio-node). Place it in the **Methods** section after the inheritance sentence and before node-specific methods (if any).

### InteractivePlayground — live audio demo with controls

Use when you have a `useXxxPlayground` hook in `src/components/InteractivePlayground/`.

```tsx
import InteractivePlayground from '@site/src/components/InteractivePlayground';
import { useOscillatorPlayground } from
  '@site/src/components/InteractivePlayground/OscillatorExample/useOscilatorPlayground';

<details open>
<summary><code>OscillatorNode</code> interactive playground</summary>
<InteractivePlayground usePlayground={useOscillatorPlayground} />
</details>
```

To add a new playground: create `src/components/InteractivePlayground/MyExample/useMyPlayground.ts`.

### InteractiveExample — side-by-side code + rendered output

```tsx
import MyExample from '@site/src/examples/MyExample/MyExample';
import MyExampleSrc from '!!raw-loader!@site/src/examples/MyExample/MyExample';

<InteractiveExample component={MyExample} src={MyExampleSrc} />
```

Source file (`src/examples/MyExample/MyExample.tsx`) must be a React component that works on web (via `react-native-web`).

### Admonitions (built-in Docusaurus)

```mdx
:::note
Informational note.
:::

:::tip
Helpful tip.
:::

:::caution
Something to watch out for.
:::
```

### Collapsible blocks

```mdx
<details open>
<summary>Click to expand</summary>

Content here.

</details>
```

### Math (LaTeX)

Inline: `$f = -\frac{\text{sampleRate}}{2}$`
Block: `$$formula$$`

## Adding a New Node Doc Page

1. Identify the category folder: `docs/sources/`, `docs/effects/`, `docs/analysis/`, etc.
2. Create `docs/<category>/my-node.mdx`.
3. Use the structure above: frontmatter → imports → Constructor → Example → Properties → Methods → Remarks.
4. Set `sidebar_position` relative to neighboring files (check their positions first).
5. Sidebar updates automatically on next `yarn start` / `yarn build`.

## Adding a New Guide / Tutorial

1. Create `docs/guides/my-guide.mdx`.
2. Use `<InteractiveExample>` to embed working code + rendered output.
3. Use `<InteractivePlayground>` for live parameter control.
4. Add the example code in `src/examples/MyGuide/`.

## Property Table Format

```mdx
| Name | Type | Default value | Description |
| :----: | :----: | :----: | :------- |
| `frequency` <ReadOnly /> | `AudioParam` | `440.0` | Oscillation frequency in Hz. |
| `type` | `OscillatorType` | `'sine'` | Waveform shape. |
```

`<ReadOnly />` — property cannot be reassigned (AudioParam wrappers are always ReadOnly).
`<Optional />` — constructor option or parameter is optional.
`<IOS />` / `<Android />` — feature is platform-specific.
