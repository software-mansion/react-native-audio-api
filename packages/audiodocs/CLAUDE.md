...
```
packages/audiodocs/
├── docs/                  # "Next" version — all documentation pages (MDX), edited here
...
├── versioned_docs/        # "Latest" version — released snapshot, do not hand-edit
├── versioned_sidebars/    # Sidebars per released version
...
The site ships two versions, configured under `presets.classic.docs.versions`:

| Version | Source | URL | Notes |
|---|---|---|---|
| `Latest` | `versioned_docs/version-latest/` | `/docs/...` | Default; mirrors the `docs/stable` branch, which documents the published npm release |
| `Next` | `docs/` | `/docs/next/...` | Tracks `main`; shows an "unreleased" banner |

**Always edit `docs/`.** `versioned_docs/version-latest/` is a mirror of `docs/stable` — hand-edits
there are overwritten on the next sync. A doc fix that must reach the live site before the next
release belongs on `docs/stable` first, then gets synced down.

### Syncing `Latest` from `docs/stable`

```bash
cd <repo root>
node -e "require('fs').rmSync('packages/audiodocs/versioned_docs/version-latest',{recursive:true,force:true})"
mkdir -p packages/audiodocs/versioned_docs/version-latest
git fetch origin docs/stable
git archive origin/docs/stable packages/audiodocs/docs \
  | tar -x --strip-components=3 -C packages/audiodocs/versioned_docs/version-latest
```

Run this when `docs/stable` moves — i.e. at release time, not per PR.

Two things the sync does **not** carry over, so check them after:

- **Static assets.** `static/` is shared by both versions, so any image `docs/stable` references
  must also exist on `main` (e.g. `static/img/oscillator-waves.png`, used only by `Latest`).
- **Anchor strictness.** `docs/stable` leaves `onBrokenAnchors` at Docusaurus's default (`warn`);
  this site sets it to `throw`. Anchors that are quietly broken on the live stable site will fail
  the build here. Fix them in the snapshot and report them upstream so `docs/stable` gets the fix too.

### Cross-document links must be relative in `docs/`

In `docs/` (the `Next` version), use relative file paths ending in `.mdx`, never absolute
`/docs/...` URLs:

```mdx
[`AudioParam`](../core/audio-param.mdx)              ✅ resolves within the reader's version
[`gain`](./gain-node.mdx#properties)                 ✅ anchors work the same way
[`AudioParam`](/docs/core/audio-param)               ❌ always jumps to Latest, even from Next
```

Absolute `/docs/...` paths are still correct for static assets under `static/docs/`
(e.g. `![](/docs/recorder/info-plist.png)`).

`versioned_docs/version-latest/` still uses absolute `/docs/...` links, inherited from
`docs/stable`. That is harmless there — `Latest` is served at `/docs/`, so those links stay inside
their own version. Do not "fix" them; the next sync would revert the change.

`onBrokenLinks` and `onBrokenAnchors` are set to `throw`, so `yarn build` catches a bad relative
path — but it cannot catch an absolute link leaking a Next reader into Latest.
