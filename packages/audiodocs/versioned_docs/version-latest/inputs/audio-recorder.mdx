...
import { Optional, ReadOnly, MobileOnly, Android } from '@site/src/components/Badges';
...
A recording that survives task removal can only be controlled from the notification, so give the user a way out: enable the notification's [stop action](/docs/system/recording-notification-manager#native-action-handling) (`showStopAction: true`) or let them [swipe the notification away](/docs/system/recording-notification-manager#swiping-the-notification-away) (`dismissible: true`). Both stop the recording natively even when no JS is reachable.
...
<!-- -->
    ...
    <tr>
      ...
      <td>
        <div class="text-content">
          Returns the [FileInfo](#fileinfo) stashed by the last successful stop that produced output files, or `null` if there is none. That includes JS [stop](#stop), the Android [recording notification](/docs/system/recording-notification-manager#native-action-handling), and [deactivating the iOS audio session](/docs/system/audio-manager#setaudiosessionactivity) mid-recording. Consume-once: the stash is cleared on read, so a second call returns `null`.
