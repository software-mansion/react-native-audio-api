...
Deactivating the session while a recording is in progress or paused stops that recording first, since an inactive session would corrupt its output. The finalized files are then available through [`AudioRecorder.consumeLastRecordingResult()`](/docs/inputs/audio-recorder#consumelastrecordingresult). Prefer calling [`AudioRecorder.stop()`](/docs/inputs/audio-recorder#stop) yourself before deactivating, so the file info arrives through its promise.
...
In React components, the [`useSystemVolume`](../react/system-volume.mdx) hook enables the observation and subscribes to the event for you.
