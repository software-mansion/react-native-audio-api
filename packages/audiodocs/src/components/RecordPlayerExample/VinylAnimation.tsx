...
export default function VinylPlayer(): React.ReactElement {
  ...
  const [isSliderPressed, setSliderPressed] = useState(false);
  ...
  const playSound = async () => {
    if (!audioContextRef.current || !audioBufferRef.current || bufferSourceRef.current) {
      return;
    }
  ...
  useEffect(() => {
    ...
    // scene, renderer, camera, renderer
    ...
    const coverGeometry = new THREE.BoxGeometry(coverWidth, coverBoxHeight, coverDepth);
    const coverMaterials = [
      boxMaterial, // right face
      boxMaterial, // left face
      boxMaterial, // top face
      invisibleMaterial, // bottom face (open box)
    ...
    const verticalTrimGeom = new THREE.BoxGeometry(trimThickness, coverBoxHeight, trimThickness);
    const cornerX = coverWidth / 2 - trimThickness / 2;
    const cornerZ = coverDepth / 2 - trimThickness / 2;

    const trims = [];
    trims.push(new THREE.Mesh(verticalTrimGeom, trimMaterial)); // front-right
    trims[0].position.set(cornerX, coverBoxHeight / 2, cornerZ);
    trims.push(new THREE.Mesh(verticalTrimGeom, trimMaterial)); // front-left
    trims[1].position.set(-cornerX, coverBoxHeight / 2, cornerZ);
    trims.push(new THREE.Mesh(verticalTrimGeom, trimMaterial)); // back-right
    trims[2].position.set(cornerX, coverBoxHeight / 2, -cornerZ);
    trims.push(new THREE.Mesh(verticalTrimGeom, trimMaterial)); // back-left
    trims[3].position.set(-cornerX, coverBoxHeight / 2, -cornerZ);

    const fbTrimGeom = new THREE.BoxGeometry(coverWidth, trimThickness, trimThickness);
    const lrTrimGeom = new THREE.BoxGeometry(trimThickness, trimThickness, coverDepth - 2 * trimThickness);
    
    const topY = coverBoxHeight - trimThickness / 2;
    const bottomY = trimThickness / 2;

    trims.push(new THREE.Mesh(fbTrimGeom, trimMaterial)); // top-front
    trims[4].position.set(0, topY, cornerZ);
    trims.push(new THREE.Mesh(fbTrimGeom, trimMaterial)); // top-back
    trims[5].position.set(0, topY, -cornerZ);
    trims.push(new THREE.Mesh(lrTrimGeom, trimMaterial)); // top-right
    trims[6].position.set(cornerX, topY, 0);
    trims.push(new THREE.Mesh(lrTrimGeom, trimMaterial)); // top-left
    trims[7].position.set(-cornerX, topY, 0);

    trims.push(new THREE.Mesh(fbTrimGeom, trimMaterial)); // bottom-front
    trims[8].position.set(0, bottomY, cornerZ);
    trims.push(new THREE.Mesh(fbTrimGeom, trimMaterial)); // bottom-back
    trims[9].position.set(0, bottomY, -cornerZ);
    trims.push(new THREE.Mesh(lrTrimGeom, trimMaterial)); // bottom-right
    trims[10].position.set(cornerX, bottomY, 0);
    trims.push(new THREE.Mesh(lrTrimGeom, trimMaterial)); // bottom-left
    ...
    const animate = () => {
      ...
      if (coverGroupRef.current && stringRef.current) {
        ...
        coverGroupRef.current.position.y += (targetCoverY.current - currentY) * 0.1;
  ...
  useEffect(() => {
    targetCoverY.current = coverHeight;

    if (filterRef.current && gainRef.current) {
      const maxCoverHeight = 7;
      const ratio = coverHeight / maxCoverHeight;
      filterRef.current.frequency.value = 200 + ratio * (8000 - 200);
      gainRef.current.gain.value = ratio * 0.5;
    }

    if ((isSliderPressed || coverHeight > 0) && !bufferSourceRef.current) {
      playSound();
    }
    if (!isSliderPressed && coverHeight === 0) {
      stopSound();
    }
  }, [coverHeight, isSliderPressed]);
