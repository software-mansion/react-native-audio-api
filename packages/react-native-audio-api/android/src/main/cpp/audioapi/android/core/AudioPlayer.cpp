...
void AudioPlayer::onErrorAfterClose(oboe::AudioStream *stream, oboe::Result error) {
  ...
  // Capture before rebuild: cleanup clears the stream that was expected to run.
  ...
  // requestStart only — do not call resume() (keeps resume free of rebuild side effects).
