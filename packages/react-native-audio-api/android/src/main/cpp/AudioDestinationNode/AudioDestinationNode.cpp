...
void AudioDestinationNode::normalize(void *audioData, int32_t numFrames, int channelCount) {
    ...
    std::for_each(outputBuffer, outputBuffer + numFrames * channelCount, [maxValue](float &sample) {
        sample /= maxValue;
    });
}
