...
BiquadFilterNode::BiquadFilterNode(AudioContext *context)
    : AudioNode(context) {
    frequencyParam_ = std::make_shared<AudioParam>(context, 350.0, MIN_FILTER_FREQUENCY, MAX_FILTER_FREQUENCY);
    detuneParam_ = std::make_shared<AudioParam>(context, 0.0, -MAX_DETUNE, MAX_DETUNE);
    QParam_ = std::make_shared<AudioParam>(context, 1.0, -MAX_FILTER_Q, MAX_FILTER_Q);
    gainParam_ = std::make_shared<AudioParam>(context, 0.0, MIN_FILTER_GAIN, MAX_FILTER_GAIN);
...
void BiquadFilterNode::setNormalizedCoefficients(float b0, float b1, float b2, float a0, float a1,
                                            float a2) {
...
void BiquadFilterNode::setLowpassCoefficients(float frequency, float Q) {
    ...
    setNormalizedCoefficients(2 * alpha, 4 * alpha, 2 * alpha, 1.0f, -2 * gamma, 2 * beta);
}
...
void BiquadFilterNode::setBandpassCoefficients(float frequency, float Q) {
    ...
    setNormalizedCoefficients(alpha, 0.0f, -alpha, 1.0f + alpha, -2 * k, 1.0f - alpha);
}
...
void BiquadFilterNode::setLowshelfCoefficients(float frequency, float gain) {
    ...
    float w0 = M_PI * frequency;
    float alpha = 0.5f * std::sin(w0) * std::sqrt((A + 1 / A) * (1 / 1.0f - 1) + 2);
    float k = std::cos(w0);
...
void BiquadFilterNode::applyFilter() {
    ...
    float normalizedFrequency = frequencyParam_->getValueAtTime(currentTime) / NYQUIST_FREQUENCY;
    ...
    switch (type_) {
        case FilterType::LOWPASS:
            setLowpassCoefficients(normalizedFrequency, QParam_->getValueAtTime(currentTime));
            break;
        case FilterType::HIGHPASS:
            setHighpassCoefficients(normalizedFrequency, QParam_->getValueAtTime(currentTime));
            break;
        case FilterType::BANDPASS:
            setBandpassCoefficients(normalizedFrequency, QParam_->getValueAtTime(currentTime));
            break;
        case FilterType::LOWSHELF:
            setLowshelfCoefficients(normalizedFrequency, gainParam_->getValueAtTime(currentTime));
            break;
        case FilterType::HIGHSHELF:
            setHighshelfCoefficients(normalizedFrequency, gainParam_->getValueAtTime(currentTime));
            break;
        case FilterType::PEAKING:
            setPeakingCoefficients(normalizedFrequency, QParam_->getValueAtTime(currentTime), gainParam_->getValueAtTime(currentTime));
            break;
        case FilterType::NOTCH:
            setNotchCoefficients(normalizedFrequency, QParam_->getValueAtTime(currentTime));
            break;
        case FilterType::ALLPASS:
            setAllpassCoefficients(normalizedFrequency, QParam_->getValueAtTime(currentTime));
            break;
        default:
            break;
    }
...
void BiquadFilterNode::process(
    ...
    for (int i = 0; i < numFrames; i++) {
        ...
        auto output = static_cast<float>(b0 * input + b1 * x1 + b2 * x2 - a1 * y1 - a2 * y2);
