...
private:
    ...
    static FilterType fromString(const std::string& type) {
        ...
        if (lowerType == "lowpass") return FilterType::LOWPASS;
        if (lowerType == "highpass") return FilterType::HIGHPASS;
        if (lowerType == "bandpass") return FilterType::BANDPASS;
        if (lowerType == "lowshelf") return FilterType::LOWSHELF;
        if (lowerType == "highshelf") return FilterType::HIGHSHELF;
        if (lowerType == "peaking") return FilterType::PEAKING;
        if (lowerType == "notch") return FilterType::NOTCH;
        if (lowerType == "allpass") return FilterType::ALLPASS;
    ...
    static std::string toString(FilterType type) {
        switch (type) {
            ...
            default: throw std::invalid_argument("Unknown filter type");
        }
    }
