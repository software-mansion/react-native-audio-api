...
- (instancetype)initWithReceiverBlock:(AudioReceiverBlock)receiverBlock bufferLength:(int)bufferLength sampleRate:(float)sampleRate
{
  if (self = [super init]) {
    ...
    self.inputFormat = [[AVAudioFormat alloc] initWithCommonFormat:AVAudioPCMFormatFloat32 sampleRate:devicePrefferedSampleRate channels:1 interleaved:NO];
    self.outputFormat = [[AVAudioFormat alloc] initWithCommonFormat:AVAudioPCMFormatFloat32 sampleRate:sampleRate channels:1 interleaved:NO];
...
- (OSStatus)processAudioInput:(const AudioBufferList *)inputData withFrameCount:(AVAudioFrameCount)frameCount atTimestamp:(const AudioTimeStamp *)timestamp {
  ...
  if (inputSampleRate != outputSampleRate) {
    AVAudioPCMBuffer *inputBuffer = [[AVAudioPCMBuffer alloc] initWithPCMFormat:self.inputFormat frameCapacity:frameCount];
    
    memcpy(inputBuffer.mutableAudioBufferList->mBuffers[0].mData,
    ...
    AVAudioPCMBuffer *outputBuffer = [[AVAudioPCMBuffer alloc] initWithPCMFormat:self.audioConverter.outputFormat frameCapacity:outputFrameCount];
