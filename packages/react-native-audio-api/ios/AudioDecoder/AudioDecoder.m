...
- (instancetype)initWithSampleRate:(int)sampleRate {
  if (self = [super init]) {
    ...
    self.format = [[AVAudioFormat alloc] initWithCommonFormat:AVAudioPCMFormatFloat32 sampleRate:self.sampleRate channels:2 interleaved:NO];
  }
  return self;
}
...
- (const AudioBufferList *)decodeWithFilePath:(NSString *)path {
  ...
  self.buffer = [[AVAudioPCMBuffer alloc] initWithPCMFormat:[audioFile processingFormat] frameCapacity:[audioFile length]];
...
- (void)convertFromFormat:(AVAudioFormat *)format {
  ...
  AVAudioPCMBuffer *convertedBuffer = [[AVAudioPCMBuffer alloc] initWithPCMFormat:self.format frameCapacity:(AVAudioFrameCount)self.buffer.frameCapacity];
