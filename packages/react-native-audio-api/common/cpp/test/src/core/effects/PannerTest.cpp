...
TEST_P(PannerStereoParametrizedTest, PanModulatesInputStereoCorrectly) {
  ...
  for (size_t i = 0; i < FRAMES_TO_PROCESS; ++i) {
    ...
    if (azimuth <= 0) {
      EXPECT_NEAR(
          (*resultBuffer->getChannelByType(AudioBuffer::ChannelLeft))[i],
          (step + step * params.expectedGainL) * totalGain,
          TOLERANCE);
      EXPECT_NEAR(
          (*resultBuffer->getChannelByType(AudioBuffer::ChannelRight))[i],
          (step * params.expectedGainR) * totalGain,
          TOLERANCE);
    } else {
      EXPECT_NEAR(
          (*resultBuffer->getChannelByType(AudioBuffer::ChannelLeft))[i],
          (step * params.expectedGainL) * totalGain,
          TOLERANCE);
      EXPECT_NEAR(
          (*resultBuffer->getChannelByType(AudioBuffer::ChannelRight))[i],
          (step + step * params.expectedGainR) * totalGain,
          TOLERANCE);
    }
  }
}
