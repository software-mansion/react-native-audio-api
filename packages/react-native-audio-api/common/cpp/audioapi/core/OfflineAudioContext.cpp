  ...
  if (totalRenderDuration <= when) {
    throw std::runtime_error(
      "cannot schedule a suspend at " +
        std::to_string(when) +
        " seconds becuase it is greater than " +
        "or equal to the total " +
        "render duration of " +
        std::to_string(numFrames_) + " frames (" +
        std::to_string(totalRenderDuration) + 
        " seconds)"
    );
  }
  ...
  if (scheduledSuspends_.find(frame) != scheduledSuspends_.end()) {
    throw std::runtime_error(
      "cannot schedule more than one suspend at frame " +
        std::to_string(frame) + " (" + std::to_string(when) +
        " seconds)"
    );
  }
...
void OfflineAudioContext::resumeRendering() {
  ...
  std::thread([this]() {
    ...
    while (currentSampleFrame_ < numFrames_) {
      Locker locker(stateLock_);
      int framesToProcess = std::min(numFrames_ - currentSampleFrame_, RENDER_QUANTUM_SIZE);
