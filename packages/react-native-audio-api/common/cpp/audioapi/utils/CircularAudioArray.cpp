...
void CircularAudioArray::push_back(const float *data, size_t size) {
    if (size > size_) {
...
void CircularAudioArray::pop_front(float *data, size_t size) {
    if (size > size_) {
        throw std::overflow_error("size exceeds CircularAudioArray size_");
    }

    if (size > getNumberOfAvailableFrames()) {
...
void CircularAudioArray::pop_back(float *data, size_t size, size_t offset, bool skipAvailableDataCheck) {
  if (size > size_) {
    throw std::overflow_error("size exceeds CircularAudioArray size_");
  }
...
size_t CircularAudioArray::getNumberOfAvailableFrames() const {
    return vWriteIndex_ >= vReadIndex_ ? vWriteIndex_ - vReadIndex_ : size_ - vReadIndex_ + vWriteIndex_;
}
