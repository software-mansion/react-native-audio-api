...
AudioBufferBaseSourceNodeHostObject::AudioBufferBaseSourceNodeHostObject(
    ...
    playbackRateParam_(std::make_shared<AudioParamHostObject>(node->getPlaybackRateParam())),
    onPositionChangedInterval_(node->getOnPositionChangedInterval()),
    inputLatency_(node->getInputLatency()),
    outputLatency_(node->getOutputLatency()) {
  addGetters(
