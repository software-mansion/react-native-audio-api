...
AudioBufferSourceNodeHostObject::~AudioBufferSourceNodeHostObject() {
  ...
  setOnLoopEndedCallbackId(0, true);
}
...
void AudioBufferSourceNodeHostObject::setOnLoopEndedCallbackId(uint64_t callbackId, bool gcEvent) {
  auto handle = node_->handle;
  auto event = [handle, node = audioBufferSourceNode_, callbackId](BaseAudioContext &) {
    node->setOnLoopEndedCallbackId(callbackId);
  };

  audioBufferSourceNode_->unregisterOnLoopEndedCallback(onLoopEndedCallbackId_);
  if (gcEvent) {
    audioBufferSourceNode_->scheduleGCEvent(std::move(event));
  } else {
    audioBufferSourceNode_->scheduleAudioEvent(std::move(event));
  }
