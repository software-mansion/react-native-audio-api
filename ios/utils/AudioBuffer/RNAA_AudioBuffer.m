...
- (float *)getChannelDataForChannel:(int)channel {
    if (channel < 0 || channel >= self.numberOfChannels) {
        @throw [NSException exceptionWithName:NSInvalidArgumentException reason:@"Channel index out of bounds" userInfo:nil];
    }
...
- (void)setChannelData:(int)channel data:(float *)data length:(int)length {
    ...
    if (length != self.length) {
        @throw [NSException exceptionWithName:NSInvalidArgumentException reason:@"Data length does not match buffer length" userInfo:nil];
    }
...
- (RNAA_AudioBuffer *)copyBuffer {
    RNAA_AudioBuffer *outputBuffer = [[RNAA_AudioBuffer alloc] initWithSampleRate:self.sampleRate length:self.length numberOfChannels:self.numberOfChannels];
...
- (RNAA_AudioBuffer *)mixWithOutputNumberOfChannels:(int)outputNumberOfChannels {
    ...
    switch (self.numberOfChannels) {
        case 1:
            if (outputNumberOfChannels == 2) {
                RNAA_AudioBuffer *outputBuffer = [[RNAA_AudioBuffer alloc] initWithSampleRate:self.sampleRate length:self.length numberOfChannels:2];
        ...
        case 2:
            if (outputNumberOfChannels == 1) {
                RNAA_AudioBuffer *outputBuffer = [[RNAA_AudioBuffer alloc] initWithSampleRate:self.sampleRate length:self.length numberOfChannels:1];
    ...
    @throw [NSException exceptionWithName:NSInvalidArgumentException reason:@"Unsupported number of channels" userInfo:nil];
}
